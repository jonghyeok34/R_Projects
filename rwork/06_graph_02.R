...
legend(0.5,1,c("MON","TUE","WED","THU","FRI"),
       cex=0.6, fill=rainbow(length(p1)))
...
#ggplot2 Package
  ...
  library(ggplot2)
  korean <-read.table(
    "./data/06_ggplot2/학생별국어성적_new.txt",header=T,sep=",")
  korean
  
  ggplot(korean,
         aes(x=이름,y=점수))+geom_point()
  ...
  # -- 
  score_kem <- read.csv("./data/06_ggplot2/학생별과목별성적_국영수_new.csv",header=T)
  score_kem
  
  
    #arrange함수 이용해 이름 과목순으로 정렬
    library(plyr)
  ...
  #3) geom_segment()함수
  score <- read.table("./data/06_ggplot2/학생별전체성적_new.txt",header=T,sep=",")
  ...
  ggplot(score,aes(x=영어,
                   y=reorder(이름,영어))) +
    geom_segment(aes(yend=이름),
                 xend=0,color="blue") +
  ...
  #5.geom_line()함수
    ...
    three <-read.csv("./06_ggplot2/학생별과목별성적_3기_3명.csv",header=T)
    three
  ...
  #예6 geom_area
    dis <-read.csv("./data/06_ggplot2/1군전염병발병현황_년도별.csv",stringsAsFactors=F)
    dis    
    
    # 그래프 출력 – geom_line() 
    ggplot(dis,
           aes(x=년도별,y=장티푸스,group=1))+
      geom_line()
    ...
    #그래프 출력 – geom_area() ->color,fill,alpha속  
    ggplot(dis,aes(x=년도별,y=장티푸스,group=1))+
